...
def gLab_output_to_numpy(output_file):
    ...
    with open(output_file, 'r') as f:
        ...
        lines = f.readlines()
        neu = np.array(
            [list(map(float, l.split()[NEU_START:NEU_END])) for l in lines if re.match('OUTPUT*', l)])  # .reshape(3,-1)
...
def download_and_save_file(url, file_path):
    def download(url):
        ...
        verify = os.name == 'nt'  # windows check
        if 'rinex' in url: verify = False
...
def download_ionex(dates_list=None, agencies_list=None, download_folder=ION_root,
                   log_filename='download_ionex.log'):
    if agencies_list is None:
        agencies_list = ['c1p', 'igs', 'ckm']
